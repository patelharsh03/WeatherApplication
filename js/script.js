...
function fetchWeatherByCoordinates(lat, lon) {
    fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${apiKey}&units=metric`)
        .then(response => {
            if (!response.ok) throw new Error('Unable to fetch data for the current location');
            return response.json();
        })
        .then(data => {
            displayWeather(data);
    ...
    forecastDates.forEach(date => {
        ...
        forecastCard.innerHTML = `
            ...
            <div class="flex items-center">
                <img src="https://openweathermap.org/img/wn/${forecast.weather[0].icon}.png" alt="${forecast.weather[0].description}" class="w-12 h-12 mr-2" />
                <div>
